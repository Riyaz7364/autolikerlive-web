...
(async () => {
    ...
    const urlPath = cleanCliArg(process.argv[2]);
    const searchKey = cleanCliArg(process.argv[3]);
    const singleValue = cleanCliArg(process.argv[4]);
    const minLength = parseInt(cleanCliArg(process.argv[5]), 10);
    ...
    const searchKeys = searchKey
        ...
        .map((k) => k.trim())
        .filter(Boolean);
    const singleValues = singleValue
        .split(",")
        .map((k) => k.trim())
        .filter(Boolean);
    ...
    const cookieString =
        "datr=rIFQatDE9dpl0EZXY00qLfwf; sb=rIFQaiNkY9efKokacHTCG4a_";
    ...
    await cluster.task(async ({ page }) => {
        ...
        try {
            ...
            fs.writeFileSync("page.html", pageSource, "utf8");
