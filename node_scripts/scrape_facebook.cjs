...
function findObjectsWithMinChildren(obj, targetKey, minChildren = 5, results = []) {
    if (typeof obj !== 'object' || obj === null) return results;

    for (const key in obj) {
        const value = obj[key];
        if (key === targetKey && typeof value === 'object' && Object.keys(value).length > minChildren) {
            results.push(value);
        }
...
(async () => {
    ...
    if (!urlPath || !searchKey || isNaN(minLength)) {
        console.error('Usage: node script.js <url_path> <json_key1,json_key2,...> <min_child_count>');
        process.exit(1);
    }
    ...
    const cookieString = 'datr=jum6aHwMZh28krUMEAzX1inS; sb=jum6aHRMjgMVebvQZxVXNToe';
    ...
    await cluster.task(async ({ page }) => {
        ...
        try {
            await page.setUserAgent('Mozilla/5.0 (platform; rv:geckoversion) Gecko/geckotrail Firefox/firefoxversion');
            ...
            page.on('response', async res => {
                ...
                if (contentType.includes('text/html')) {
                    try {
                        ...
                        console.error('Error reading HTML response:', err.message);
                    }
                }
            });
                    ...
                    try {
                        ...
                        for (const rawKey of searchKeys) {
                            ...
                            if (!resultsPerKey[rawKey]) {
                                const matches = findObjectsWithMinChildren(data, key, minLength);
                                if (matches.length > 0) {
                                    resultsPerKey[rawKey] = isMulti ? matches : matches[0];
                                    matched = true;
                                }
                            }
                        }
