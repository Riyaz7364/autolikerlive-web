<div wire:poll.5000ms='checkInbox'>
            ...
            <div class="inbox-empty-msg">
                <svg width="92" height="94" viewBox="0 0 92 87" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <path class="emptyInboxRotation"
                        ...
                        stroke="#5eead4" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"></path>
                    <path class="emptyInboxRotation"
    ...
    @else
        @foreach (collect($this->messages['messages'])->sortByDesc('receivedAt') as $message)
            @php
                $msgId = is_array($message) ? $message['id'] : (is_object($message) ? $message->id : '');
            @endphp
            <a href="{{ url('') }}/message/view/{{ $msgId }}" class="msg-row">
                <span class="msg-cell msg-main">
                    <span class="msg-sender">
                        <span class="msg-dot {{ $message['is_seen'] ? 'is-not-active' : 'is-active' }}"></span>
                        {{ $message['from'] }}
                    </span>
                    <small class="msg-subject">{{ $message['subject'] }}</small>
                </span>
                <span class="msg-cell msg-email">{{ $message['from_email'] }}</span>
                <span class="msg-cell msg-time">{{ \Carbon\Carbon::parse($message['receivedAt'])->diffForHumans() }}</span>
            </a>
        @endforeach
    @endif
</div>
