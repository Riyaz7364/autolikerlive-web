        ...
        @foreach (collect($this->messages['messages'])->sortByDesc('receivedAt') as $message)
            <a href="{{ url('') }}/message/view/{{ is_array($message) ? $message['id'] : (is_object($message) ? $message->id : '')  }}" class="viewLink d-flex mb-3">
                ...
                <div class="col-6 mx-2">
                    <small
                        class="text-dark ${email.is_seen == true ? '' : 'font-weight-bold'}">{{ $message['subject'] }}</small>
                    <span class="d-flex text-muted"
                        style="font-size: small;font-style: italic;">{{ $message['from_email'] }}</span>
                </div>
                ...
                font-style: italic;
            ">
                    {{ \Carbon\Carbon::parse($message['receivedAt'])->diffForHumans() }}
                </div>
