...
class TempMailController
{
   ...
   public function allMessages(Request $request)
   {
       ...
       try {
           ...
           // Removed dd($client) debug statement
           $messages = $time == 0
               ? $folder->query()->to($email)->limit(50)->get()  // Added limit for performance
               : $folder->query()->to($email)->since(Carbon::now()->subDays($time)->format('d-M-Y'))->limit(50)->get();
   ...
   protected function formatResponse($email, $messages, $domainPrefix, $prefix, $client)
    {
        ...
        for ($i=0; $i < count($messages); $i++) {
            ...
            $response['messages'][$i] = $this->formatMessage($messages[$i], $domainPrefix, $prefix);
        ...
        if ($client !== null) {
            try {
                ...
                // Silently fail if disconnect has issues
            }
        }
