...
class InstagramController extends Controller
{
    ...
    public function claimBonus(Request $request)
    {
        ...
        $completedPromotions = Activity::where('uid', $user->uid)->whereDate('created_at', Carbon::today())->count();
    ...
    public function boostSubmit(Request $request)
    {
        ...
        if (preg_match('/\/(p|reel)\/([A-Za-z0-9_-]+)\/?/', $request['link'], $match) && $user->loginType != 'FB') {
            $post_id = $match[2];
        } elseif (preg_match('/\/share\/([^\/]+)/i', $request['link'], $matches) && $user->loginType == 'FB') {
            $request = Http::get($request['link']);
            $post_id = $request->effectiveUri();
    ...
    public function placeViewOrder(Request $request)
    {
        ...
        if ($validator->fails()) {
            return redirect()
                ->back()
                ->withErrors(['msg' => 'Video Link is Required']);
        }
        ...
        if ($json['success'] == true) {
            ...
            if (isset($response->error)) {
                return redirect()
                    ->back()
                    ->withErrors(['message' => 'Something wrong contact admin!']);
            }
    ...
    private function loadTimer(Request $request, $userId = null)
    {
        $timeRemains = 0;
        $userIp = $userId ?? getUserIP($request);
    ...
    private function saveTimer(Request $request)
    {
        ...
        TiktokTimer::updateOrCreate(['id' => $userIp], ['timer' => $timerExpiration]);
